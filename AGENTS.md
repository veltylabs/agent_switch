...
- **Append-only audit log, single derived "current state":** `AgentSwitch` is INSERT-only — no
  `db.Update`/`db.Delete` call exists anywhere in this module, ever. "Current status" is not a
  column anyone mutates; it is whichever row has the largest `changed_at`, read fresh on every
  `get_agent_status` call.
- **`model.IDGenerator` cannot reconstruct a timestamp from an ID.** The pre-harness version derived
  `changed_at` by parsing the unixid-generated `id` (`uid.Parse(id)`) — a capability specific to
  `tinywasm/unixid`, not part of `model.IDGenerator` (`NewID() string` only). The harness version
  adds an explicit `changed_at int64` column, set via `github.com/tinywasm/time.Now()` at insert
  time, and reads "current status" via `ORDER BY changed_at DESC LIMIT 1` instead of `ORDER BY id
  DESC` — `id` is no longer guaranteed time-sortable once it comes from a generic
  `model.IDGenerator` (that guarantee was `unixid`-specific, not part of the port). No local
  "timestamp-parsing" interface was declared to route around this — see `docs/PLAN.md`'s identity
  stage for the full rationale, and double-check this resolution before relying on it.
- **Owns its own schema migration.** `New()` calls `ddl.CreateTable(&AgentSwitch{})` behind the
  `db.RawConn().(ddl.Compiler)` type assertion like every other module — it is not a read-only
  consumer of a table another module or the composition root created.
  Only 2 columns exist. `AgentSwitchModel` never gets a `form/input` widget — no
  `model.Definition` in this module is DB-backed but rendered.
- **No `view.Presenter` (`NewView`) in this module by design.** A single append-only boolean log
  with no natural "list → select → edit" workflow does not fit `view.New`'s list+select+save+delete
  shape: toggling always appends a new row, it never edits a selected one, and there is no "list all
  history" op to serve as the required `ListOp`. This is a judgment call, not a placeholder omission
  — double-check it if a future stage adds a full history-listing op, which would change the
  calculus.
